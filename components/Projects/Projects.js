...
import PENN from '../../static/images/penn.png';
...
const services = [

  {
    ...
    projectOverview: [{projectNumber: 1, projectName: 'Full Stack App', projectDescription: 'Built a custom web application using a Laravel, React, PostgreSQL, and Nginx stack. Application allows for internal business users to stream content from a salesforce database table to a custom user interface. Enabled server side rendering for a single page application using Inertia.js middleware. Connected Laravel authentication to LDAP active directory services and built custom session management. Well documented codebase with swagger integration and react-storybook component libraries.'}]
